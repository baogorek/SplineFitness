...
export function CircuitWorkout({ onModeChange }: CircuitWorkoutProps) {
  ...
  useEffect(() => {
    const activePhases: Phase[] = ["ready", "transition", "timing", "input"]
  ...
  const comboTimer = useTimer({
    ...
    onTick: (remainingSeconds) => {
      ...
      if (lastAnnouncedExerciseRef.current !== currentExerciseIndex && lastAnnouncedExerciseRef.current !== -1) {
        ...
        const equipmentDesc = exerciseEquipment[currentExercise.id]
        setTransitionEquipmentNote(equipmentDesc ? `${equipmentDesc}` : "")
        ...
        audio.speak(equipmentDesc ? "Transition. Grab your sand weight" : "Transition")
      ...
      if (secondsLeftInExercise === 15 && exerciseDuration >= 25 && lastAnnouncedCueRef.current !== cueKey) {
        ...
        if (isLastExercise) {
          ...
          audio.speak(`15 seconds left. You will have ${nextTransition} seconds to transition to ${nextName}.`)
        }
  ...
  const handleResume = useCallback(() => {
    ...
    setExerciseEquipment(pendingResume.exerciseEquipment || getExerciseEquipment())
  ...
  const handleSetupComplete = useCallback(
    (
      ...
      choices: Record<string, "main" | "alternative">,
      equipment: Record<string, string>
    ) => {
      setActiveWorkout(variant)
      setExerciseSettings(settings)
      setExerciseChoices(choices)
  ...
  const handleResumeTransition = useCallback(() => {
    transitionTimer.start()
    startTransitionCountdown(transitionTimer.remainingSeconds)
  }, [transitionTimer, startTransitionCountdown])
  ...
  const handleStartCombo = useCallback(() => {
    ...
    if (firstExercise) {
      ...
      setTransitionExerciseName(name)
      const equipmentDesc = exerciseEquipment[firstExercise.id]
      setTransitionEquipmentNote(equipmentDesc ? `${equipmentDesc}` : "")
      transitionExerciseDurationRef.current = exerciseSettings[firstExercise.id]?.durationSeconds || 60
      transitionTimer.reset()
      setPhase("transition")
      transitionTimer.start()
      const speechText = equipmentDesc ? `Get ready for ${name}. Grab your sand weight` : `Get ready for ${name}`
        ...
        if (exercise) {
          setWeakLinks((prev) => [
            ...prev,
            {
              ...
              exerciseName: resolveExerciseName(exercise, exerciseChoices),
    ...
    <div className="min-h-screen flex flex-col pb-4">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex-shrink-0 px-4 pt-4 pb-2 bg-background">
            ...
            <div className="flex flex-col items-center gap-4 py-8">
              ...
              <div className="flex items-center gap-3">
                {transitionTimer.isRunning ? (
                  <button
                    onClick={handlePauseTransition}
                    className="px-4 py-2 rounded-full text-sm font-medium bg-amber-600 text-white hover:bg-amber-700 transition-colors"
                  >
                    Not Ready
                  </button>
                ) : (
                  <button
                    onClick={handleResumeTransition}
          ...
          {phase === "ready" && currentCombo && (() => {
            const equippedExercises = currentCombo.subExercises.filter(s => exerciseEquipment[s.id])
            if (equippedExercises.length === 0) return null
            return (
              <div className="rounded-lg bg-amber-500/15 border border-amber-500/30 p-3 mb-3">
                <p className="text-sm font-medium text-amber-500">
                  Equipment needed: {equippedExercises.map(s => `${exerciseEquipment[s.id]} for ${resolveExerciseName(s, exerciseChoices)}`).join(", ")}
                </p>
              </div>
            )
          })()}
